...
(function (root) {
  ...
  /** Split resume/JD text into short semantic units: bullets, then sentences as fallback. */
  function splitIntoUnits(text) {
    ...
    for (const line of lines) {
      // Long lines with no bullet structure: split into sentences.
      if (line.length > 200) {
    ...
    return units.filter((u) => u.length > 2 && u.length < 400);
  }
  ...
  /** Splits JD text into required vs preferred requirement units, dropping soft-skill-only asks. */
  function parseJDRequirements(jdText, softTerms) {
    ...
    for (const rawLine of lines) {
      ...
      if (!line) continue;
      if (REQUIRED_HEADING_RE.test(line) && line.length < 60) { section = "required"; continue; }
      if (PREFERRED_HEADING_RE.test(line) && line.length < 60) { section = "preferred"; continue; }
