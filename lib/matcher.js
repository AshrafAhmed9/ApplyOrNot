...
(function (root) {
  ...
  // real requirement. Used only as the last-resort fallback below, when no dictionary
  // skill term is found at all.
  ...
  /**
   * Compiles a finite skills dictionary into a single matcher regex, longest terms first
   * so e.g. "React Native" is preferred over matching just "React" inside it.
   */
  function buildSkillsMatcher(skillTerms) {
    const sorted = [...skillTerms].sort((a, b) => b.length - a.length);
    const escaped = sorted.map((t) => t.replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/\s+/g, "\\s+"));
    // Custom alphanumeric boundary instead of \b: \b fails for terms ending in symbols
    // (e.g. "C++" is almost never followed by a word character, so \b right after it
    // rarely matches) and, without ANY boundary at all, short terms like "C" or "AI"
    // would match as bare substrings inside unrelated words ("MetricsLand" contains "c").
    const pattern = escaped.map((e) => `(?<![A-Za-z0-9])(?:${e})(?![A-Za-z0-9])`).join("|");
    return new RegExp(pattern, "gi");
  }

  /** Every dictionary skill term literally present in text, in original casing, deduped. */
  function extractSkillMentions(text, skillsRegex) {
    if (!skillsRegex) return [];
    const found = [];
    const seen = new Set();
    skillsRegex.lastIndex = 0;
    let m;
    while ((m = skillsRegex.exec(text))) {
      const key = m[0].toLowerCase();
      if (!seen.has(key)) {
        seen.add(key);
        found.push(m[0]);
      }
      if (m.index === skillsRegex.lastIndex) skillsRegex.lastIndex++; // guard against zero-width loops
    }
    return found;
  }
   ...
   * Primary method is a real skills-dictionary match — a positive-match approach, so a
   * company name or generic word can never be shown (they simply won't be in the
   * dictionary). Only falls back to the capitalization heuristic when no dictionary term
   * is found at all. Scoring always uses the full original text; this only affects display.
   */
  function extractKeyPhrase(unit, skillsRegex) {
    const trimmed = unit.trim();

    if (skillsRegex) {
      const hits = extractSkillMentions(trimmed, skillsRegex);
   ...
   * skillsRegex: compiled skills-dictionary matcher (via buildSkillsMatcher), used for
   *   display-only keyword extraction of gaps/strengths — optional, falls back gracefully
   */
  function getVerdict({ resume, jd, jdVector, jdUnitVectors, domainCentroids, softTerms, skillsRegex }) {
    ...
    const reqGaps = reqScored.filter((r) => !r.covered).map((r) => extractKeyPhrase(r.text, skillsRegex));
    const prefGaps = prefScored.filter((r) => !r.covered).map((r) => extractKeyPhrase(r.text, skillsRegex));
    const strengths = reqScored.filter((r) => r.covered).map((r) => extractKeyPhrase(r.text, skillsRegex));
