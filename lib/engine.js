...
(function (root) {
  ...
  async function processResumeText(rawText) {
    ...
    const resumeVector = MatcherLib.meanVector(vectors);
    const classification = MatcherLib.classifyDomain(resumeVector, centroids);
  ...
  async function scoreJD(jdText, resumes) {
    ...
    const jdVector = jdUnitVectors.length
      ? MatcherLib.meanVector(jdUnitVectors.map((u) => u.vec))
      : (await embedTexts([jdText.slice(0, 1000)]))[0];

    const jd = { text: jdText };
    const verdictFn = (resume) =>
      MatcherLib.getVerdict({ resume, jd, jdVector, jdUnitVectors, domainCentroids, softTerms });

    return MatcherLib.pickBestResume(resumes, verdictFn);
  }

  const EngineLib = { getSoftTerms, getDomainAnchors, getDomainCentroids, embedTexts, processResumeText, scoreJD };
