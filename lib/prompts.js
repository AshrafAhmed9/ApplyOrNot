...
//
// The verdict is a TWO-STAGE decision (see lib/llm.js getVerdict):
//   Stage 1 — DETECT: "is this a real job posting?" — run WITHOUT the candidate profile, so the
//     model cannot use topical overlap with the candidate's own skills/projects as false evidence
//     that a page is a posting. Not a posting → NO_JD, Stage 2 skipped.
//   Stage 2 — FIT: "APPLY or SKIP?" — only for confirmed postings.
(function (root) {
  // ---------------- Stage 1: job-posting detection (profile-blind) ----------------
  const DETECT_SYSTEM_PROMPT = `You classify a single web page. You are given the page title and the text extracted from the page. Decide EXACTLY ONE thing: is this page a genuine JOB POSTING — one specific open position, published by or on behalf of an employer, that a real person could apply to and be hired for?
...
Output JSON only: {"isJobPosting": true} or {"isJobPosting": false}. No other text.`;
  ...
  const DETECT_FEW_SHOT = [
    { input: { title: "Backend Engineer (New Grad) — Acme Corp", text: "About the role: build scalable APIs and async processing. Responsibilities: you will design services, work with databases, deploy to cloud. Requirements: 0-2 years. Apply now." }, output: { isJobPosting: true } },
    { input: { title: "ApplyOrNot", text: "The role description is for an AI recruiter tool, not a job opening. The candidate's profile is used as input for the tool's evaluation process." }, output: { isJobPosting: false } },
    { input: { title: "lsmtree-db — GitHub repository", text: "A persistent LSM-tree key-value store implemented from scratch, with crash-safety via write-ahead logging. Build instructions, benchmarks, design notes, license." }, output: { isJobPosting: false } },
    { input: { title: "React – A JavaScript library for building user interfaces", text: "Documentation, tutorials, API reference. Get started, learn the fundamentals, community resources." }, output: { isJobPosting: false } },
  ];
...
gaps: up to 3, real concerns/missing hard requirements only, empty array if none. No markdown, no text outside the JSON object.`;
  ...
  const DETECT_SYSTEM_PROMPT_FULL = `${DETECT_SYSTEM_PROMPT}
...
Be concise — short phrases, not sentences, in each array. No markdown, no commentary outside the JSON object.`;
  ...
  const PromptsLib = {
    DETECT_SYSTEM_PROMPT_FULL,
    VERDICT_SYSTEM_PROMPT,
    VERDICT_FEW_SHOT,
    VERDICT_SYSTEM_PROMPT_FULL,
