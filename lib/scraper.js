// Reads the FULL text of a job-posting-shaped container from the page, fully automatically —
// the user never clicks "Read more". Two-tier strategy: precise Tier-1 selectors for known sites
// (LinkedIn, Indeed, major ATS platforms), and a Readability-based Tier-2 fallback for any other
// career site, with a whole-page last resort so there's always something to send.
//
// IMPORTANT: this file does NOT decide whether the text is actually a job description — an
// earlier regex-based "isLikelyJD" gate was removed because it produced false negatives on
// short/informally-worded real JDs (rejecting a real posting is worse than sending a non-JD page
// to the model). Classifying "is this a JD" is left entirely to the AI, which returns a NO_JD
// decision when the text isn't a job posting (see VERDICT_SYSTEM_PROMPT). The only local check
// left here is a bare content-presence threshold — is there any meaningful text at all — so a
// truly empty page still costs zero tokens.
(function (root) {
  ...
  /** Bare presence check — is there any meaningful text at all — never a JD/non-JD judgment. */
  function hasText(text) {
    return !!text && text.trim().length >= MIN_TEXT_LENGTH;
  }

  /** Best-effort posting title: helps the model anchor role/seniority even when body text is
   *  messy or truncated. Not required for a verdict — falls back gracefully to an empty string. */
  ...
  /** Last resort when no known container or Readability article is found: the cleaned whole-page
   *  text. Whether this text is actually a job posting is left entirely to the AI (NO_JD). */
  function readViaWholePage() {
    try {
      return cleanBlockText(extractBlockText(document.body));
    } catch (e) {
      return null;
    }
  }

  /** Full pipeline: expand truncated content, then extract the best-available page text via
   *  Tier-1 site selectors, the generic description guess, Readability, or (last resort) the
   *  whole page. This function never classifies whether the text is a JD — see the file-level
   *  comment — it only picks the best text and reports "none" when there's no text at all. */
    ...
    if (!hasText(text)) {
      ...
      if (hasText(genericGuess)) {
        ...
        if (hasText(fallback)) {
          ...
          const wholePage = readViaWholePage();
          if (hasText(wholePage)) {
            text = wholePage;
            tier = "tier3";
          } else {
            return { text: "", title, tier: "none", site: site?.key || "unknown" };
          }
        }
      }
    }
    return { text, title, tier, site: site?.key || "unknown" };
  }

  root.ScraperLib = { scrapeFullJD, matchSite, loadSiteConfig, hasText };
